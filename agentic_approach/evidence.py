...
Input: data/rows_adapted.jsonl (id + base/other dicts from Step 1)
Output: out/evidence.jsonl with evidence_sources, best_evidence, evidence_tier_used, row_confidence.

Process:
1. Build query from row (name, location hints, website candidates)
2. Try website candidates first (other then base): fetch, classify, extract
3. Fallback to SearchBusiness (tier B) if configured
4. Consolidate best_evidence per field; compute row_confidence
5. No label/winner decision — evidence only.
"""
...
# Tier scores: higher = more trustworthy source
# A = direct website fetch (we control the fetch); B = external search API
TIER_A_SCORE = 1.0
TIER_B_SCORE = 0.7

# Early-stop: stop fetching more URLs once we have strong evidence
NAME_THRESH = 0.7
PHONE_OR_ADDR_THRESH = 0.7
...
# Fields that, when conflicting, cap row_confidence at 0.6
CRITICAL_FIELDS = {"name", "phones", "address"}
...
def gather_from_websites(
    row: dict,
    query: dict,
    tier_score: float = TIER_A_SCORE,
    delay: float = 0.5,
) -> tuple[list[dict], dict[str, list[dict]], bool]:
    """
    Fetch up to 5 website candidates, classify each page, extract fields from relevant ones.
    Early-stops when name>=0.7 and (phone>=0.7 or address>=0.7).
    Returns (evidence_sources, extracted_per_url, early_stopped).
    """
    sources = []
    extracted_per_url = {}
    early_stopped = False
    region = _region_from_address(
        (row.get("other") or {}).get("address") or (row.get("base") or {}).get("address")
    )

    for url in query.get("website_candidates", [])[:5]:  # Limit to 5 URLs per row
        url = _normalize_url(url)
        if not url:
            continue
        status, final_url, html = fetch_url(url, timeout=10)
        time.sleep(delay)  # Be polite to servers

        classification = classify_page(html, status, final_url, query)
        source = {
            "url": final_url,
            "status": status,
        ...
        if classification != CLASS_RELEVANT:
            extracted_per_url[final_url] = {}
            continue

        fields = extract_fields(html, final_url, query, region)
        # Convert to evidence format: {field: [(value, confidence), ...]}
        evidence = {}
        for f, pairs in fields.items():
            evidence[f] = [{"value": v, "confidence": c} for v, c in pairs]
        extracted_per_url[final_url] = evidence

        # Early stop: name>=0.7 and (phone>=0.7 or address>=0.7)
        name_conf = max((c for _, c in fields["name"]), default=0)
        phone_conf = max((c for _, c in fields["phones"]), default=0)
        addr_conf = max((c for _, c in fields["address"]), default=0)
        if name_conf >= NAME_THRESH and (phone_conf >= PHONE_OR_ADDR_THRESH or addr_conf >= PHONE_OR_ADDR_THRESH):
            early_stopped = True
            break

    return sources, extracted_per_url, early_stopped


def search_business_fallback(query: dict, config: dict | None) -> tuple[list[dict], dict]:
    """
    Fallback to external SearchBusiness API (tier B) when no website evidence found.
    Stub: not implemented; would call external API with name/locality/region.
    Returns (sources, extracted_evidence).
    """
    return [], {}
...
def consolidate_evidence(
    row: dict,
    sources: list[dict],
    extracted_per_url: dict[str, dict],
    tier_b_evidence: dict | None = None,
) -> dict[str, Any]:
    """
    Merge evidence from all URLs into best value per field.
    Score = tier_score * field_confidence. Values within 10% of best are kept as conflicts.
    """
    best = {}
    field_scores = {}  # field -> [(value, tier_score * conf, source_tier), ...]

    def add_value(field: str, value: str, confidence: float, tier_score: float, tier_name: str):
        if not value or not str(value).strip():
            return
        score = tier_score * confidence
        if field not in field_scores:
            field_scores[field] = []
        field_scores[field].append((str(value).strip(), score, tier_name))

    for url, evidence in extracted_per_url.items():
        tier_score = TIER_A_SCORE
        for field, items in evidence.items():
            for item in items:
                if isinstance(item, dict):
                    ...
                    v, c = item, 0.7
                add_value(field, v, c, tier_score, "A")

    if tier_b_evidence:
        for field, items in tier_b_evidence.items():
            for item in items:
                if isinstance(item, dict):
                    v = item.get("value")
                    c = float(item.get("confidence", 0.5))
                    ...
                    v, c = item, 0.7
                add_value(field, v, c, TIER_B_SCORE, "B")

    # Pick best value per field; keep alternatives as conflicts if score within 10%
    for field, scored in field_scores.items():
        if not scored:
            continue
        scored.sort(key=lambda x: -x[1])
        best_val, best_s, best_tier = scored[0]
        conflicts = []
        for v, s, t in scored[1:]:
            if s >= best_s * 0.9 and v != best_val:
                conflicts.append({"value": v, "score": s, "tier": t})
        best[field] = {
            "value": best_val,
            "score": best_s,
            "tier": best_tier,
...
def compute_row_confidence(best_evidence: dict, has_conflicts_on_critical: bool) -> float:
    """
    Compute row_confidence (0-1): avg score * completeness.
    Capped at 0.6 if name/phones/address have conflicting values.
    """
    if not best_evidence:
        return 0.0
    fields_with_data = sum(1 for v in best_evidence.values() if v.get("value"))
    total_fields = 6  # name, phones, address, category, emails, socials
    completeness = fields_with_data / total_fields
    avg_tier = 1.0
    scores = [v.get("score", 0) for v in best_evidence.values() if v.get("value")]
    if scores:
        avg_tier = sum(scores) / len(scores)
    conf = avg_tier * completeness
    if has_conflicts_on_critical:
        conf = min(conf, 0.6)
    return round(min(1.0, conf), 3)


def process_row(row: dict, search_config: dict | None = None, delay: float = 0.5) -> dict:
    """
    Process one adapted row: build query, gather from websites, consolidate, compute confidence.
    Returns output dict for evidence.jsonl (id, base_id, evidence_sources, best_evidence, etc.).
    """
    query = build_query(row)
    sources, extracted_per_url, early_stopped = gather_from_websites(row, query, delay=delay)

    # Fallback to tier B (search API) only if no good website evidence
    tier_b_sources = []
    tier_b_evidence = {}
    if not early_stopped and (not extracted_per_url or not any(extracted_per_url.values())):
        tier_b_sources, tier_b_evidence = search_business_fallback(query, search_config or {})

    all_sources = sources + tier_b_sources
    evidence_tier_used = "B" if tier_b_sources and not sources else ("A" if sources else "none")

    best_evidence = consolidate_evidence(row, all_sources, extracted_per_url, tier_b_evidence or None)

    # Check for conflicts on critical fields (name, phones, address)
    has_conflicts = any(
        best_evidence.get(f, {}).get("conflicts")
        for f in CRITICAL_FIELDS
        if f in best_evidence
    )
    row_confidence = compute_row_confidence(best_evidence, has_conflicts)
...
def run_evidence_gathering(
    ...
    """
    Read JSONL from input_path, process each row, write evidence to output_path.
    ...
    with open(input_path, "r", encoding="utf-8") as fin, open(
        ...
        for line in fin:
            ...
            try:
                ...
                continue
            out = process_row(row, delay=delay)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Step 2: Evidence gathering. Input: rows_adapted.jsonl, Output: evidence.jsonl"
    )
    parser.add_argument(
        "--input",
        default="data/rows_adapted.jsonl",
        help="Input JSONL path (default: data/rows_adapted.jsonl)",
    )
    parser.add_argument(
        "--out",
        default="out/evidence.jsonl",
        help="Output JSONL path (default: out/evidence.jsonl)",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    if not input_path.exists():
        ...
        return 1
    count = run_evidence_gathering(
        input_path,
        Path(args.out),
        limit=args.limit,
        delay=args.delay,
    )
    print(f"Wrote {count} evidence rows to {args.out}")
    return 0
