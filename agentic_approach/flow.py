...
def run_flow(
    ...
    *,
    ...
    with open(output_path, "w", encoding="utf-8") as fout:
        for row in rows:
            out = process_row(row, delay=delay, use_llm=use_llm, llm_model=llm_model, debug=debug)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified row-by-row agentic flow. Input: rows_adapted.jsonl, Output: agentic_labels.jsonl"
    )
    parser.add_argument(
        "--input",
        default=None,
    ...
    parser.add_argument("--no-llm", action="store_true", help="Disable LLM for categories")
