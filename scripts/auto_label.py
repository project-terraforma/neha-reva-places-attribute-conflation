#!/usr/bin/env python3
"""
Strict auto-labeler for Project A Golden Dataset (place-level label).

[CHANGE: now reads/writes JSON instead of CSV - undo to revert to CSV]
Reads analysis/inspection/golden/golden_dataset.json, computes per-row scores
(base vs alt across phones, websites, addresses, categories), and updates
the scores section and label in-place.

Score meanings: 0=base, 1=alt, 2=abstain

Usage:
  python scripts/auto_label.py

Notes:
- This is intentionally conservative: many rows will abstain.
- It does NOT use confidence.
- It assumes values are JSON-ish or Python-literal-ish strings.
"""
